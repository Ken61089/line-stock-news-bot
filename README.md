...
2. **兜底 — MOPS t163sb04 全市場季報彙總**。有些公司只在期限內申報財報、不另發重訊,
   ...
   跑一次補漏,一季一次請求就拿到全上市/上櫃的累計 EPS。回補歷史也走這條。
...
⚠️ **三個要知道的限制**:
- 累計相減的單季 EPS 在**當年度有現金增資/配股**時會失真(前後季加權平均股數不同,
  EPS 本質上不可線性相減)。表上的單季營收/淨利相減則不受影響,可拿來對照。
- 表格裡帶 `*` 的是彙總表回補的列,**只有 EPS**、沒有營收與淨利(彙總表各業別欄位數不同,
  一般業 29 欄、金融業 21 欄,只有最後一欄的累計基本每股盈餘位置穩定)。
  走重訊詳細頁的列才有完整營收/淨利。
