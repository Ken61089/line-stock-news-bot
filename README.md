...
所以程式用**累計歸屬母公司淨利 ÷ 累計基本EPS** 反推每季的加權平均股數(實測台積電
259.3 億股、廣達 38.6 億股、鴻海 139.1 億股,與實際股本相符),再分兩種情況處理 ——
**兩種增資的性質完全不同,不能混為一談**:

**① 現金增資 / CB 轉股(有對價)**:**不追溯調整**,各期累計值都是自己期間的實際加權,
基準一致,只是相減會有誤差。程式改用 **單季淨利 ÷ 本季加權股數** 回推(淨利相減不受
股數影響,除以本季自己的股數就回到同一基準),該格標 `~`:
...
**② 配股 / 股票分割(無對價)**:IFRS 要求**追溯調整**所有比較期間,但 MOPS 彙總表不會
回頭改我們庫裡已存的舊值,於是新舊基準混在同一張表。這種才會警示,而且**連跨年比較都
會誤導**,所以直接給換算倍數:

```
⚠️ 2025Q3 起股數基準已變(5.13→20.54 億股),更早的累計 EPS ÷4.0 才同基準;跨這條線的數字不可直接比較
```

真實案例(2026-08 回補時撞到):國巨 2025 年 1 股拆 4 股,Q3 起追溯成分割後基準。
硬相減會得到 `8.22 − 20.51 = -12.29` 的假虧損,改用淨利回推後是 **3.09**,
與追溯調整後的 `8.22 − 5.13` 完全吻合。年度小計也一樣要小心:國巨 2024 全年 38.13
(分割前)vs 2025 全年 11.51(分割後),看起來像崩掉六成,實際 38.13÷4 = 9.53,是成長的。

門檻:相鄰兩季股數差 >3% 就改用淨利回推(①);>20% 才判定為基準斷點並警示(②)。
Q1 的單季直接等於累計、不做相減,所以不會有這個問題,也不標記。

⚠️ **另外兩個限制**:
- **自結數 vs 正式財報**:同一季會有兩個版本(自結先出、未經會計師核閱)。自結**只在該季
  還沒有任何數字時才寫**,正式財報或彙總表一到就自動蓋過去,表上標 `自`。
  ⚠️ 自結有**單月**版本(報導期間 115/06/01~115/06/30),結束月同樣是 6,所以解析時
  **起始日必須是 01/01** 才認 —— 否則會把一個月的數字當成半年累計寫進去。
...
## 🔔 盤中事件提醒(Notion「5. 事件提醒」)

條件到了就把你**當初寫好的關注內容**原封不動推回眼前,不必在盤勢亂的當下再想一次
該看什麼。
...
**跌/漲**是當日漲跌幅(以昨收為基準)、**破/站上**是絕對價位。

觸發長這樣:

```
🔔 大盤 當日跌1000點
大盤 現在 44,396.70(-214.90點, -0.48%)  13:33:00
────────
打BBU、記憶體、3324
```

**資料源**:TWSE MIS 即時報價 `mis.twse.com.tw/stock/api/getStockInfo.jsp`
(免費、無 Cloudflare、**一次請求可帶多檔**,所以整批提醒只花一個 request;需帶 Referer)。
上市/上櫃前綴 `tse_`/`otc_` 不同,新增提醒時自動判斷一次並存進 Notion。

**排程**:週一~週五 09:00-13:45 每 15 分鐘。13:45 那次拿到的是收盤值,收盤才跌破也接得住。

⚠️ **額度控制**(LINE 免費 200 則/月):
- 只推**本人私訊**,不退回群組 —— 推群組是按成員數計費(3 人群推 1 則算 3 則)。
  沒設 `LINE_ALLOWED_USER_ID` 就寧可不推。
- **推一次就自動停用**,要再用打「開提醒 N」。條件一旦成立往往會**持續成立**
  (大盤跌破某點位後可能連跌好幾天),設成每日一次的話每天都會再吃一則額度。
  想改行為就到 Notion 把「重複方式」換成「每日一次」或「每次都推」。
- 每日推播上限 `ALERT_DAILY_CAP`(預設 5 則),防止盤勢劇烈時把額度吃光。
- 主動推播總開關(`推播關閉`)關閉時一樣不推。

⚠️ **非交易日**:API 在休市時會回**上一個交易日**的報價,所以要比對回傳的 `d`(交易日期)
是不是今天 —— 不比對的話週末每 15 分鐘都會拿昨天的跌幅重複觸發。
