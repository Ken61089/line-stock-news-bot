...
| `產業報告`(或 `券商報告`/`個股報告`/`研究報告`) | 個股產業報告 | 個股、**概念股(題材族群)**、報告日期、出具券商、目標價、近期營收、時間軸、報告總結(含**利多/利空訊號**) |
...
每次記錄**個股新聞 / 產業新聞 / 產業報告**時,機器人會自動把「概念 ↔ 個股」寫進一張 **「概念股主表」** 分頁(**概念導向**:一列一個概念,右邊列出所屬個股),並在回覆標出受影響的概念與本次新增的股票:

```
🏷️ 概念股主表已更新:
• CPO(2檔):6442 光聖, 4979 華星光 (新增:4979 華星光)
• 光通訊(1檔):4979 華星光 (新概念)
```

主表欄位:`概念/題材 | 成分股 | 個股數 | 首次出現 | 最近出現`。一列一個概念,右邊累積該概念的所有個股,一眼就知道「某概念有哪些股」。
- 成分股以**代號**為去重鍵(如 6442),所以「光聖」「6442 光聖」會視為同一檔(前提是新聞有帶代號)。
- 之後就能查:`查 CPO 概念有哪些股` 或 `查 光聖屬於哪些概念`。
- 更新主表失敗不會影響新聞存檔(只是當次少了標籤摘要)。

#### 概念名稱統一:「概念清單」白名單

為避免同一題材被寫成不同名稱(CPO / 共同封裝光學 / 共封裝光學…)而讓主表碎片化,系統維護一張 **「概念清單」** 分頁當白名單。AI 抽概念時會**優先對照清單、沿用標準名稱**,清單沒有的才自行命名。

- 第一次執行會自動用內建種子(台股常見題材,約 75 項)建立這張分頁。
- 你可以**直接在 Sheet 編輯**(增刪題材、改標準名)。⚠️ 清單有快取,**改完需重啟服務**才生效:`npx zeabur@latest service restart --id <SERVICE_ID> --env-id <ENV_ID> -y -i=false`。

### 🛠️ 即時更正(在 LINE 直接改錯)

AI 偶爾會標錯,可以用指令即時更正,不用開電腦。**在 LINE 打「說明」(或 `help`)可隨時叫出完整指令總表。**

**改概念股主表**(打「主表」看用法):
```
主表 加 CPO:6442 光聖          把某檔加進某概念
主表 移除 光通訊:2330          把某檔從某概念移除
主表 移除股 8111               把某檔從「所有」概念移除(代號打錯用)
主表 改股 8111:6442 光聖        股號打錯一次改掉(所有概念裡的 8111 換成 6442 光聖)
主表 合併 共同封裝光學:CPO      概念合併/改名(A 併入 B)
主表 刪除 半導體               刪掉整個概念
```
...
改股 8111:6442 光聖
```
會掃過**所有分頁**的個股欄(新聞時程動態庫、產業動態、個股產業報告的個股欄 + 概念股主表的成分股),把 8111 全部換成 6442 光聖,並回報每個分頁改了幾處。
> 差別:`改股 …`(無「主表」前綴)= 改**所有分頁**;`主表 改股 …` = 只改**概念股主表**。

**改某一筆新聞**(可改更早的;打「改」看用法):
```
改 <分類> <關鍵字> <欄位>:<新值>
例:改 產業報告 全新 目標價:500元
```
會在該分類分頁中,找**最近一筆**含關鍵字的資料列,更新指定欄位(欄位名可用部分字,如「目標價」對到「券商目標價」)。

---

## 🗓️ 時程與盯盤(寫進 Notion 行事曆)

除了寫 Google Sheet,還能把「時程事件」與「隔日盯盤筆記」寫進 **Notion**(需設 `NOTION_TOKEN` 等變數,見 `.env.example`)。
...
```
- AI 自動拆出個股、日期、事件類型,寫進 Notion 時間軸並依代號**關聯個股**(概念族群由 Notion rollup 自動帶出)。
- 事件標題會**自動帶上公司名**(例:「2330 台積電 法說會」),行事曆一眼看得出是哪家。
- 個股主表**查無該檔時會自動新增再關聯**(可用 `AUTO_CREATE_STOCK=0` 關閉);自動新增的個股尚無概念,之後補「隸屬概念」即可。
...
3661 世芯 觀察缺口
```
- 用「關注」開頭,每行一筆,預設記到**隔天**(可用「關注 今天 …」指定當日),事件類型為「每日關注」。
- 隔天早上 07:00 會**隨群組推播**帶出。與時程不同:關注**不會**自動新增個股(避免主表被盯盤筆記灌爆)。

---

## 📊 財報 EPS 自動歸檔(Notion「4. 財報 EPS 庫」)

追蹤股的每季每股盈餘會**全自動**進表,不用手動填。

```
EPS 2330      → 近 6 季單季/累計每股盈餘表
EPS 台積電    → 也可用主表裡的名稱
EPS 回補      → 用 MOPS 彙總表補齊追蹤股近 8 季(跑約 5 分鐘,跑完不推播)
EPS 用法      → 說明
```

回覆長這樣(庫裡存 8 季,顯示只給近 6 季):
...
小計**按年份**給,不做跨年滾動四季。值直接取該年最大季的累計 EPS(季報累計本來就是
年初至今),所以即使該年只顯示到 Q4 一列,小計仍是正確的全年值。

第一列沒有 `*`:那是當天重訊詳細頁抓的(累計 4.28 扣掉 Q1 的 1.80 = 單季 2.48);
其餘帶 `*` 的是彙總表回補。

**兩條資料線**(都是純程式解析,不用 AI、不吃 Firecrawl 額度):

1. **主線 — MOPS 重訊詳細頁**(即時)。既有的每 2 小時重訊掃描裡,只要追蹤股發出
   「公告本公司◯◯年度第◯季合併財務報告業經董事會決議通過」這類重訊,就自動 POST
   進詳細頁,抽出固定編號格式的欄位:報導期間起訖、累計營業收入、累計淨利、
   **1月1日累計至本期止基本每股盈餘**。董事會一過就有數字,不必等財報彙總表。
2. **兜底 — MOPS t163sb04 全市場季報彙總**。有些公司只在期限內申報財報、不另發重訊,
   靠主線會漏;所以每晚 22:40 在財報公布窗口(3/15-4/10、5/1-5/25、8/1-8/24、11/1-11/24)
   跑一次補漏,一季一次請求就拿到全上市/上櫃的累計 EPS 與歸屬母公司淨利。回補歷史也走這條。
   ⚠️ 這頁依業別分成好幾張表、**欄位位置都不一樣**(一般業 30 欄、金融/證券/保險 18~23 欄),
   「淨利歸屬於母公司業主」在第 11~23 欄之間浮動,所以要逐張表讀自己的表頭定位,
   不能寫死索引;唯一穩定的只有「基本每股盈餘」永遠是最後一欄。

**單季 EPS 怎麼算**:季報申報的每股盈餘本來就是「1月1日累計至本期止」的**累計數**,
所以單季 = 本季累計 − 前一季累計;Q1 累計即單季;**Q4 沒有「第四季重訊」**,
年報期間是 01/01-12/31,故 Q4 = 年度累計 − Q3 累計。任何一季的累計值變動,
該檔所有季會自動重算。

### 增資自動偵測(單季 EPS 什麼時候不能信)

EPS 的分母是**加權平均**流通股數。公司年度中途現金增資/CB 轉股/配股,前後兩季分母基準
不同,累計值相減出來的單季 EPS 就會失真 —— 例如 Q1 賺 1 億(1 億股,EPS 1.00)、Q2 初增資
到 2 億股後 Q2 再賺 1 億,上半年累計 EPS 是 2÷1.5=1.33,相減得 0.33,但真實單季是 0.50。

所以表格會自己抓出這種年份:用**累計歸屬母公司淨利 ÷ 累計基本EPS** 反推每季的加權平均
股數(實測台積電 259.3 億股、廣達 38.6 億股、鴻海 139.1 億股,與實際股本相符),年度內
變動超過 5% 就在表尾加一行:

```
⚠️ 2026 加權股數 1.20→1.38 億股(+15%),該年單季 EPS 僅供參考
```

只判斷年度**內**的變動 —— Q1 的單季 EPS 直接等於累計、不必相減,失真只發生在 Q2~Q4。

⚠️ **另外兩個限制**:
- **公司根本不申報單季 EPS**。季報只有「1月1日累計至本期止」的累計數,這是法規格式,
  重訊詳細頁與彙總表都一樣。所以真實單季 EPS 沒有任何官方來源可抄,只能相減估算
  (券商網站與看盤軟體多數也是這樣算的)。
- 主線只處理「這一輪新出現」的重訊。機器人重啟會重建基準,當天稍早的重訊不會補抓 —— 
  這正是兜底那條線存在的理由。
